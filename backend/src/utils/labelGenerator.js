...
function generateDeliveryLabel(order, res) {
  try {
    ...
    const orderIdStr = order._id ? order._id.toString() : 'UNKNOWN';
    const invoiceNo = `INV-${new Date(order.createdAt || Date.now()).getFullYear()}-${orderIdStr.slice(-6).toUpperCase()}`;
    const orderIdLabel = `ORD-${orderIdStr.slice(-8).toUpperCase()}`;
    ...
    doc.text(`Order ID: ${orderIdLabel}`, 40, doc.y, { align: 'left' });
    ...
    doc.font(fontNormal).fontSize(11)
      .text('Kalyani Fashion Hub', 40, splitY + 17)
    ...
    const customerPhones = [addr.mobile1, addr.mobile2].filter(Boolean).join(', ');
    ...
    if (order.address) {
      if (addr.street) doc.text(addr.street, 300, doc.y);
    ...
    (order.items || []).forEach(item => {
      doc.text(`Product: ${item.name || 'Unknown Item'}`, 40, doc.y);
      doc.text(`Size: ${item.size || '-'}`, 40, doc.y);
      doc.text(`Color: ${item.color || '-'}`, 40, doc.y);
      doc.text(`Quantity: ${item.quantity || 1}`, 40, doc.y);
      doc.moveDown(0.5);
    });
    ...
    const capStatus = ordStatus.charAt(0).toUpperCase() + ordStatus.slice(1);
